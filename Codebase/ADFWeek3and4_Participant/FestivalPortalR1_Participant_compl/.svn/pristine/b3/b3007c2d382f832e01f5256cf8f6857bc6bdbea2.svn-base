...
public class EventDAO {
	...
	public ArrayList<Event> showAllEvents() throws ClassNotFoundException,
		...
		while (resultSet.next()) {
			...
			// eventList =resultSet.getObject();

			// eventList.add(resultSet.)
	 ...
	 * If no records are updated or a SQL Error occurs throw a
	 * FERSGenericException with the <br/>
	 ...
	 * Check the count: If the count is 1 or more, return TRUE, else return
	 * FALSE
	 * 
	 * @param visitor
	 *            (type Visitor)
