...
public class EventServiceImpl implements EventFacade {
	...
	public List<Event> getAllEvents() {
		...
		return eventList; // This return value will vary depending on the coding
							// based on Todos above. It may be
							// ArrayList<Event[]>
	}
	...
	public boolean checkEventsofVisitor(Visitor visitor, int eventid) {
		...
		try {
			...
			status = eventDAO1.checkEventsofVisitor(v, 1);
		} catch (ClassNotFoundException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		} catch (SQLException e) {
		...
		return status; // replace "false" with the "status" variable used in the
						// code
						// in 'TODO' section
	}
	...
	public void updateEventDeletions(int eventid) {
		...
		try {
			eventDAO2.updateEventDeletions(id);
		} catch (ClassNotFoundException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		} catch (SQLException e) {
